...
class WABetaInfo {
  async home() {
    try {
      const {
        data
      } = await axios.get(`https://${process.env.DOMAIN_URL}/api/tools/web/html/v1?url=https://wabetainfo.com`);
  ...
  async read(url) {
    try {
      const {
        data
      } = await axios.get(`https://${process.env.DOMAIN_URL}/api/tools/web/html/v1?url=${encodeURIComponent(url)}`);
      const $ = cheerio.load(data);
  ...
  async search(q) {
    try {
      ...
      const {
        data
      } = await axios.get(`https://${process.env.DOMAIN_URL}/api/tools/web/html/v1?url=${encodeURIComponent(encLink)}`);
      const $ = cheerio.load(data);
      return $('article[id^="post-"]').map((_, el) => ({
        title: $(".entry-title a", el).text().trim(),
