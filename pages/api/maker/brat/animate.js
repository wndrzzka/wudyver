...
class BratService {
  ...
  async processInput(input) {
    try {
      const urls = input.split(" ").map(word => `https://${process.env.DOMAIN_URL}/api/maker/brat/v1?text=${encodeURIComponent(word)}`);
      const {
        data
      } = await this.client.post(`https://${process.env.DOMAIN_URL}/api/tools/onlineconvert`, {
        url: urls,
        duration: 1
      }, {
        headers: {
