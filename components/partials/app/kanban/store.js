  ...
  reducers: {
    sort: (state, action) => {
      ...
      if (source.droppableId !== destination.droppableId) {
        ...
        const destColumn = state.columns.find(column => column.id === destination.droppableId);
    ...
    addTask: (state, action) => {
      const column = state.columns.find(column => column.id === state.openTaskId);
      column.tasks.push(action.payload);
    },
    deleteTask: (state, action) => {
      const column = state.columns.find(item => item.tasks.findIndex(innerItem => innerItem.id === action.payload) !== -1);
      if (column) {
    ...
    updateTask: (state, action) => {
      const column = state.columns.find(item => item.tasks.findIndex(innerItem => innerItem.id === action.payload.id) !== -1);
      if (column) {
