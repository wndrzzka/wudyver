...
class Downloader {
  ...
  async search(query) {
    try {
      ...
      const {
        data: html
      } = await axios.get(`https://${process.env.DOMAIN_URL}/api/tools/web/html/v2?url=${link}`, {
  ...
  async detail(url) {
    try {
      const {
        data: html
      } = await axios.get(`https://${process.env.DOMAIN_URL}/api/tools/web/html/v2?url=${encodeURIComponent(url)}`, {
        headers: this.headers
      });
      const $ = cheerio.load(html);
      ...
      const {
        data: media
      } = await axios.get(`https://${process.env.DOMAIN_URL}/api/tools/web/html/v2?url=${encodeURIComponent(videoUrl)}`, {
        headers: this.headers
      });
      return {
